...
I2C i2c(p26, p27); //for the nordic
// I2C i2c(p9, p10); //for the lpc
// The Nunchuk I2C address
#define NUNCHUK_ADDRESS 0xA4

// #if ARDUINO >= 100
// #define I2C_READ() i2c.read()
// #define I2C_WRITE(x) i2c.write(x)
// #else
// #define I2C_READ() i2c.receive()
// #define I2C_WRITE(x) i2c.send(x)
// #endif

// #define I2C_START(x) i2c.beginTransmission(x)
// #define I2C_STOP() i2c.endTransmission(true)
...
char rcv[6];
    ...
    cmd[1] = 0x55;
    printf("1\n");
    if (i2c.write(NUNCHUK_ADDRESS, (const char*)cmd, sizeof(cmd))) return 0;
    // i2c.start();
    // if (i2c.write(NUNCHUK_ADDRESS)) return 0;
    // if (i2c.write(cmd[0])) return 0;
    // if (i2c.write(cmd[1])) return 0;
    // i2c.stop();
    cmd[0] = 0xFB;
    cmd[1] = 0x00;
    printf("2\n");
    if (i2c.write(NUNCHUK_ADDRESS, cmd, sizeof(cmd))) return 0;
    // i2c.start();
    // if (i2c.write(NUNCHUK_ADDRESS)) return 0;
    // if (i2c.write(cmd[0])) return 0;
    // if (i2c.write(cmd[1])) return 0;
    ...
    cmd[0] = 0x40;
    cmd[1] = 0x00;
    if (i2c.write(NUNCHUK_ADDRESS, cmd, 2)) return 0;
#endif
    ...
    printf("Ident: ");
    cmd[0] = 0xFA;
    if (i2c.write(NUNCHUK_ADDRESS, cmd, 1)) return 0;
    // i2c.start();
    // if (i2c.write(NUNCHUK_ADDRESS)) return 0;
    // if (i2c.write(cmd[0])) return 0;
    // // if (i2c.write(cmd[1])) return 0;
    // i2c.stop();
    if (i2c.read(NUNCHUK_ADDRESS, rcv, 6)) return 0;
    // i2c.start();
    // i2c.write(NUNCHUK_ADDRESS | 1);
    ...
    wait_us(1000); // Wait for serial transfer, before loop()ing
#endif
return 1;
}
...
static uint8_t nunchuk_read() {
    ...
    // i2c.start();
    // if (i2c.write(NUNCHUK_ADDRESS)) return 0;
    // if (i2c.write(0x00)) return 0;
    // i2c.stop();
    if (i2c.read(NUNCHUK_ADDRESS, nunchuk_data, 6, false)) return 0;
    // i2c.start();
    // if (i2c.write(NUNCHUK_ADDRESS | 1)) return 0;
    // nunchuk_data[0] = i2c.read(0);
    // nunchuk_data[1] = i2c.read(0);
    // nunchuk_data[2] = i2c.read(0);
    // nunchuk_data[3] = i2c.read(0);
    // nunchuk_data[4] = i2c.read(0);
    // nunchuk_data[5] = i2c.read(0);
    // i2c.stop();
#ifdef SAM
    ...
    // I2C_WRITE(0x00);
    if (i2c.write(NUNCHUK_ADDRESS, 0x00, 1)) return 0;
    // i2c.start();
    // if (i2c.write(NUNCHUK_ADDRESS)) return 0;
    // if (i2c.write(0x00)) return 0;
    // i2c.stop();
...
static void nunchuk_print() {
    ...
    // Serial.print("\n");
    printf("%d, ", nunchuk_joystickX());
    printf("%d, ", nunchuk_joystickY());
    printf("%d, ", nunchuk_accelX());
    printf("%d, ", nunchuk_accelY());
    printf("%d, ", nunchuk_accelZ());
    printf("%d, ", nunchuk_buttonZ());
    printf("%d\n", nunchuk_buttonC());
#endif
}
